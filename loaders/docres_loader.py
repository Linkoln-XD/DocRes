...
class DocResTrainDataset(data.Dataset):
    def __init__(self, dataset={}, img_size=512,):
        ...
        self.background_paths = glob.glob('/home/linkoln-xd/python_projects/dtd/*/*/*')
    ...
    def data_processing(self,task,data):
        ...
        if task=='deblurring':
            ...
            bm = np.load(os.path.join(self.im_path,data['gt_path'])).astype(np.float64)  #-> 0-448 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! np.float -> float
            '''
##################################################
            # Тут решение проблемы с разной размерностью массивов
            # Если bm 2D (один канал) - нужно создать второй канал
            if len(bm.shape) == 2:
                # Создаем 2 канала
                # Возможно, оба канала содержат одинаковые данные?
                # Или второй канал нужно вычислить?
                # На эти вопросы китайские коллеги благополучно не ответят нигде ))))
                bm_3d = np.zeros((bm.shape[0], bm.shape[1], 2))
                bm_3d[:, :, 0] = bm  # x-координаты
                bm_3d[:, :, 1] = bm  # y-координаты (или другая логика) Опять же хз что они имели в виду
                bm = bm_3d

            # Если bm 3D с одним каналом (H, W, 1)
            elif len(bm.shape) == 3 and bm.shape[2] == 1:
                bm_3d = np.zeros((bm.shape[0], bm.shape[1], 2))
                bm_3d[:, :, 0] = bm[:, :, 0]
                bm_3d[:, :, 1] = bm[:, :, 0]
                bm = bm_3d

            # Если bm уже 3D с 2+ каналами
            elif len(bm.shape) == 3 and bm.shape[2] >= 2:
                # Берем только первые 2 канала
                bm = bm[:, :, :2]
####################### Дальше код не менялся в этой функции
            '''
