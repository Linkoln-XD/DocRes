...
def train(args):
    ...
    '''
    datasets_setting = [
        {'task': 'deblurring', 'ratio': 1,
         'im_path': '/media/linkoln-xd/7CFC-DA07/Datasets_Krek0204/train_datasets/deblurring',
         'json_paths': ['/home/linkoln-xd/python_projects/DocRes/train.json']},
    ]
    '''
    # for dewarping
    datasets_setting = [
        {
            'task': 'dewarping',
            'ratio': 1,
            'im_path': '/home/linkoln-xd/python_projects/dewarping',
            'json_paths': ['/home/linkoln-xd/python_projects/fordew/train.json']
        }
    ]
    ...
    trainloaders = [{'task':datasets_setting[i],'loader':data.DataLoader(dataset=datasets[i], shuffle=True, batch_size=args.batch_size, num_workers=2, pin_memory=True,drop_last=True),'iter_loader':iter(data.DataLoader(dataset=datasets[i], shuffle=True, batch_size=args.batch_size, num_workers=2, pin_memory=True,drop_last=True))} for i in range(len(datasets))]
